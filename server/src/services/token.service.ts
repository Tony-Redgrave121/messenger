...
class TokenService {
    ...
    async saveToken(user_id: string, user_token_body: string) {
        return await index.user_tokens.create({user_token_id: uuid.v4(), user_token_body, user_id})
    }

    async deleteToken(user_token_body: string) {
        return await index.user_tokens.destroy({where: {user_token_body: user_token_body}})
    }

    async findToken(user_token_body: string) {
        return await index.user_tokens.findOne({where: {user_token_body: user_token_body}})
    }
}
