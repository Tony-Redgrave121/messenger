...
class MessengerSettingsService {
    public async fetchMessengerSettings(messenger_id: string) {
        const [messengerSettings, reactions_count, messenger] = await Promise.all([
            models.messenger_settings.findOne({
                include: [{
            ...
            models.reactions.count(),
            models.messenger.findOne({
                include: [
                    {
        ...
        if (messengerPlain.messenger_image) {
            const imagePath = path.join(__dirname, `../src/static/messengers/${messenger_id}/${messengerPlain.messenger_image}`)
    ...
    public async updateMessengerType(messenger_id: string, messenger_type: string) {
        const [updated] = await models.messenger_settings.update(
    ...
    public async updateMessengerLink(messenger_id: string) {
        ...
        const [updated] = await models.messenger.update(
    ...
    public async postMessengerReactions(messenger_setting_id: string, newReactions: string[]) {
        ...
        await models.messenger_reactions.bulkCreate(createOps)
        await models.messenger_reactions.destroy({
            where: {
        ...
        return await models.messenger_reactions.findAll({where: {messenger_setting_id}})
    }
    ...
    public async addContactsMembers(members: string[], messenger_id: string) {
        ...
        await models.members.bulkCreate(newMembersData)
    ...
    public async addMember(user_id: string, messenger_id: string) {
        await models.removed_users.destroy({where: {messenger_id, user_id}})

        const existing = await models.members.findOne({where: {messenger_id, user_id}})
    ...
    public async postRemoved(user_id: string, messenger_id: string) {
        const existing = await models.removed_users.findOne({where: {user_id, messenger_id}})
        if (!existing) {
        ...
        await models.members.destroy({where: {messenger_id, user_id}})
    ...
    public async putMessenger(messenger_id: string, messenger_name: string, messenger_desc?: string, messenger_files?: IMessengerFiles | null) {
        ...
        return models.messenger.findOne({where: {messenger_id}})
    }

    public async deleteRemoved(user_id: string, messenger_id: string) {
        const deleted = await models.removed_users.destroy({where: {messenger_id, user_id}})
    ...
    public async deleteMember(user_id: string, messenger_id: string) {
        const deleted = await models.members.destroy({where: {messenger_id, user_id}})
