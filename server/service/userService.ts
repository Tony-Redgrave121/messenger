...
class UserService {
    ...
    async fetchMessengers(user_id: string) {
        if (!user_id) return ApiError.internalServerError("An error occurred while fetching the messengers")

        const messengers = await models.messenger.findAll({
            include: [{
                model: models.member,
