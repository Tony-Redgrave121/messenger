...
const SettingButton: FC<ISettingButtonProps> = ({foo, children, text, desc, isRed}) => {
    return (
        <button
            onClick={foo}
            className={clsx(buttonStyle.SettingButton, isRed && settingButtonStyle.SettingButtonRed)}
        >
            {children}
            <p>{text} {desc?.toString && <small className={settingButtonStyle.Desk}>{desc}</small>}</p>
        </button>
    )
}
