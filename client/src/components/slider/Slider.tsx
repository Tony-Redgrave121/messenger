...
import {setState, setZoom} from "../../store/reducers/sliderReducer";
import {Link} from "react-router-dom";
import {RangeInput} from "../../rebuild/shared/ui/Input";
import {WhiteButton} from "../../rebuild/shared/ui/Button";
            ...
            <div className={style.SliderContainer} ref={refSlider}>
                <div onClick={event => event.stopPropagation()} className={style.ToolsBlock}>
                    <Link to={`/chat/${message.user.user_id}`}>
                        <LoadFile imagePath={message.user.user_img ? `users/${message.user.user_id}/${message.user.user_img}` : ''} imageTitle={message.user.user_name}/>
                ...
                {(!zoom && message.message_files && slideNumber < message.message_files.length - 1) &&
