...
const EditReactions: FC<IEditReactionsProps> = ({
    ...
    const debounceHandleReactions = useCallback(
        debounce(async reactions => {
            if (!messengerSettingsId) return;

            try {
                const updatedReactions = await postMessengerReactionsApi(
                    messengerSettingsId,
                    reactions,
                );
                                ...
                                foo={() => {
                                    if (newReactions.includes(reaction.reaction_id))
                                        setNewReactions(prev => [
                                            ...prev.filter(el => el !== reaction.reaction_id),
                                        ]);
