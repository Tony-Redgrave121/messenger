...
import {
    ...
    getExt,
    getFileName
} from "../../../rebuild/shared/lib";
import {setCurrentSlide, setSlideNumber, setZoom} from "../../../store/reducers/sliderReducer";
import MessageService from "../../../services/MessageService";
import {useParams} from "react-router-dom";

const initialMessage: IMessagesResponse = {
    message_id: '',
    message_text: '',
...
const useSlider = (refSwipe: RefObject<HTMLDivElement | null>) => {
    const [message, setMessage] = useState<IMessagesResponse>(initialMessage)
    ...
    useEffect(() => {
        ...
        const fetchMessage = async () => {
            try {
                const fetchedMessage = await MessageService.fetchMessage(messengerId, messageId, signal)
